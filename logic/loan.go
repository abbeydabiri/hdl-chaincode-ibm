...
//LoanH reads state from blockchain
func LoanH(stub shim.ChaincodeStubInterface, args []string) pb.Response {

	if len(args) != 1 {
		cErr := &utils.ChainError{FCN: utils.LoanH, KEY: "", CODE: utils.CODEUNPROCESSABLEENTITY, ERR: errors.New("Incorrect no of input args, got " + fmt.Sprintf("%v", len(args)) + " expecting Loan ID only")}
		return shim.Error(cErr.Error())
	}

	if len(args[0]) == 0 {
		cErr := &utils.ChainError{FCN: utils.LoanH, KEY: "", CODE: utils.CODEUNPROCESSABLEENTITY, ERR: errors.New("Loan ID can not be empty")}
		return shim.Error(cErr.Error())
	}
