...
//RoleH reads state from blockchain
func RoleH(stub shim.ChaincodeStubInterface, args []string) pb.Response {

	if len(args) != 1 {
		cErr := &utils.ChainError{FCN: utils.RoleH, KEY: "", CODE: utils.CODEUNPROCESSABLEENTITY, ERR: errors.New("Incorrect no of input args, got " + fmt.Sprintf("%v", len(args)) + " expecting Role ID only")}
		return shim.Error(cErr.Error())
	}

	if len(args[0]) == 0 {
		cErr := &utils.ChainError{FCN: utils.RoleH, KEY: "", CODE: utils.CODEUNPROCESSABLEENTITY, ERR: errors.New("Role ID can not be empty")}
		return shim.Error(cErr.Error())
	}
