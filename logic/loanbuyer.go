...
//LoanBuyerH reads state from blockchain
func LoanBuyerH(stub shim.ChaincodeStubInterface, args []string) pb.Response {

	if len(args) != 1 {
		cErr := &utils.ChainError{FCN: utils.LoanBuyerH, KEY: "", CODE: utils.CODEUNPROCESSABLEENTITY, ERR: errors.New("Incorrect no of input args, got " + fmt.Sprintf("%v", len(args)) + " expecting Loan Buyer ID only")}
		return shim.Error(cErr.Error())
	}

	if len(args[0]) == 0 {
		cErr := &utils.ChainError{FCN: utils.LoanBuyerH, KEY: "", CODE: utils.CODEUNPROCESSABLEENTITY, ERR: errors.New("Loan Buyer ID can not be empty")}
		return shim.Error(cErr.Error())
	}
