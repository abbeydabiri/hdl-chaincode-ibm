...
//LoanMarketShareH reads state from blockchain
func LoanMarketShareH(stub shim.ChaincodeStubInterface, args []string) pb.Response {

	if len(args) != 1 {
		cErr := &utils.ChainError{FCN: utils.LoanMarketShareH, KEY: "", CODE: utils.CODEUNPROCESSABLEENTITY, ERR: errors.New("Incorrect no of input args, got " + fmt.Sprintf("%v", len(args)) + " expecting Share ID only")}
		return shim.Error(cErr.Error())
	}

	if len(args[0]) == 0 {
		cErr := &utils.ChainError{FCN: utils.LoanMarketShareH, KEY: "", CODE: utils.CODEUNPROCESSABLEENTITY, ERR: errors.New("Share ID can not be empty")}
		return shim.Error(cErr.Error())
	}

	obj := &models.LoanMarketShare{ShareID: args[0]}
	return obj.GetHistory(stub)
}
